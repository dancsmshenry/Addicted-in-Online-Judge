...
- ```cpp
  //	前提：left被调整到mid+1，right被调整到mid
  //	关于left和right的调整
  int mid = right + (left - rihgt) / 2;
  
  int mid = left + (right - left) / 2; // 为了防止循环，必须要用这种方法
  
  //	因为如果最后的left和right分别是0和1的话，此时right被调整为1的，那么第一种方法会不断的陷入循环，导致bug发生
  //	而使用第二种方法就不会陷入循环
  //	换言之，第一种方法，会在数组数量为偶数的时候，mid会取偏向右边的那一个
  //	而第二种方法，会取偏向左边的那一个
  
  //	所以其实到底去哪一种，要看你是如何调整边界的（XD）
  ```
