...
- 快速排序的实现
  ...
  - ```cpp
    ...
    /**
    	...
    	A：不可以，对于一些数据，会因此陷入无限循环（例：9，8，7，6，5，4，3，2，1），比如说此时temp选择了1，那么就会导致i一直往右走（我这里是没有对i的返回进行限定的），最后越界报错
    	...
    	Q：为什么a[i] < temp的位置不用=
    	A：因为我们只要保证大于temp的在右边，小于temp的在左边，至于等于的，无论在左还是在右，排完序过后都会被放到中间的（temp在左边是最大的，在右边是最小的）
    **/
    ```
...
- 215、数组中的第K个最大元素
  - 写这类题的心得
  - 都是可以套用原来的模板的
  - 注意，返回值只能在left>=right的时候取到，不要想着在下面的if的过程中取到
