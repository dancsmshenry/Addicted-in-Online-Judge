...
- 顺子的概念：五张大小有顺序的牌（不能重复）
- 题意：大小王视为0，可以变为任意的数字
- 所以可以推出一些结论：如果出现重复的就false；因为大小王可以视为0，所以只要当前最大的数和最小的非零的数之差小于5-（0的数量），就可以组成顺子，参考代码
- 解法：先排序，然后再遍历一遍



- 字节面试题：判断7张牌里面是否有顺子（我对题意有两种理解）
  - 理解一：这里的顺子是指7张牌的顺子（那就和上面一样喽）
  - 理解二：顺子还是指5张牌的顺子，那就要先找到0的个数，然后依次遍历，看是否满足上面的条件（当前最大和最小的非零数的差的条件）